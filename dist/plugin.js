...
async function mergeImages(args) {
  ...
  try {
    await mergeImg([options.expectedImage, options.diffImage, options.actualImage], {direction:'vertical', margin:20})
    .then((img) => {
      console.log(img)
      img.write(path.join(CYPRESS_SCREENSHOT_DIR, args.specDirectory, `${fileName} (failed).png`));
    });
  } catch (error) {
    return { error };
  }
