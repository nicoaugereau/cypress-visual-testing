...
function takeScreenshot(subject, name, screenshotOptions) {
  let screenshotPath;
  const objToOperateOn = subject ? cy.get(subject) : cy; // save the path to forward between screenshot and move tasks

  function onAfterScreenshot(_doc, props) {
    screenshotPath = props.path;
  }

  objToOperateOn.screenshot(`${name}-actual`, { ...screenshotOptions,
    onAfterScreenshot
  }).then(() => {
